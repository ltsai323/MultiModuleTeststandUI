...
def buttonRUN():
    jobTEMPLATE = '{}.Run'
    if DEBUGMODE:
        AddJob(jobTEMPLATE.format('TEST1'),_VARS_.cmders['TEST1'].Run)
        AddJob(jobTEMPLATE.format('TEST2'),_VARS_.cmders['TEST2'].Run)
...
def buttonTEST():
    jobTEMPLATE = '{}.Test'
    if DEBUGMODE:
        AddJob(jobTEMPLATE.format('TEST1'),_VARS_.cmders['TEST1'].Test)
        AddJob(jobTEMPLATE.format('TEST2'),_VARS_.cmders['TEST2'].Test)
...
def buttonDESTROY():
    ...
    ## asdf error
    if DEBUGMODE:
        AddJob(jobTEMPLATE.format('TEST2'),_VARS_.cmders['TEST2'].Destroy)
        AddJob(jobTEMPLATE.format('TEST1'),_VARS_.cmders['TEST1'].Destroy)
...
def module_init(app):
    ...
    class OverwriteBuildInFunc_:
        from flask_socketio import SocketIO
        def __init__(self, socketIO:SocketIO):
            self.socketio = socketIO
            import collections
            self.mesglist = collections.deque(maxlen=20)
        def log_method(self,mesgUNIT:MesgHub.MesgUnit):
            print(f'send_log_to_website {mesgUNIT}')
            self.socketio.emit('bkgRunJobs', Info(mesgUNIT))
        def sleep_func(self,sleepPERIOD):
    ...
    if DEBUGMODE:
        # SSH connector to ntugrid8
        name = 'TEST1'
        sshconn = subunit.PyModuleConnectionConfig(name, '0.0.0.0', 2000)
        #sshconf = subunit.PyModuleCommandPool('../CommandPost/data/subunit_ssh_connect.daq_client.yaml')
        sshconf = subunit.PyModuleCommandPool('../CommandPost/data/subunit_ssh_connect.defaults.yaml')
        sshunit = subunit.SubUnit(sshconn,sshconf)
        _VARS_.cmders[name] = UnitStageCommander(sshunit, new_log_and_sleep)
        name = 'TEST2'
        sshconn = subunit.PyModuleConnectionConfig(name, '0.0.0.0', 2001)
        #sshconf = subunit.PyModuleCommandPool('../CommandPost/data/subunit_ssh_connect.daq_client.yaml')
        sshconf = subunit.PyModuleCommandPool('../CommandPost/data/subunit_ssh_connect.defaults.yaml')
        sshunit = subunit.SubUnit(sshconn,sshconf)
        _VARS_.cmders[name] = UnitStageCommander(sshunit, new_log_and_sleep)
