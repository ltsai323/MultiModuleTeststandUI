...
def stage_cmd_initialize(cmdPACK:JobCMDPackManager.JobCMDPack, jobSTAT:JobStatManager.JobStatMgr, terminateJOBflag=threading.Event()):
    ...
    maxV = cmdPACK.all_configs["MaximumOutputVoltage"]
    maxI = cmdPACK.all_configs["MaximumOutputCurrent"]
    controlMode = cmdPACK.all_configs["ControlMode"]
    init_cmd_before_run = [
        f'[sendcmd - maximum output voltage {maxV}]VSET1:{maxV}',
        f'[sendcmd - maximum output current {maxV}]ISET1:{maxI}',
        f'[sendcmd - set control mode {controlMode}]LOAD1:{controlMode}',
         '[sendcmd - power on]:OUTPUT1:STATE ON'
        ]
    send_rs232_mesg(
            cmdPACK.run_rm,
            cmdPACK.RecordLog,
...
def stage_cmd_stop(cmdPACK:JobCMDPackManager.JobCMDPack, jobSTAT:JobStatManager.JobStatMgr,terminateJOBflag=threading.Event()):
    ...
    while True:
        ...
        if jobSTAT.IsFinished():
            stage_cmd_exec(cmdPACK,jobSTAT,'STOP',terminateJOBflag)
def stage_cmd_destroy(cmdPACK:JobCMDPackManager.JobCMDPack, jobSTAT:JobStatManager.JobStatMgr,terminateJOBflag=threading.Event()):
    ...
    stage_cmd_exec(cmdPACK, jobSTAT, 'DESTROY', terminateJOBflag)
...
def testfunc():
    ...
    cmdpack = JobCMDPackFactory(log_center, 'data/config_rs232cmder_powersupply2.yaml')
